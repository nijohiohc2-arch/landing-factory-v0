  ...
  return `
    <div class="shell">
      ...
      <section class="hero-factory">
        ...
        <p class="lede">완벽한 한 장을 만드는 빌더가 아닙니다. 서로 다른 설득 가설을 빨리 만들어 비교합니다.</p>
...
function bindProject(id) {
  document.getElementById("regen")?.addEventListener("click", () => {
    ...
    Store.allLandings()
      ...
      .forEach((l) => Store.deleteLanding(l.id));
  ...
  document.querySelectorAll("[data-clone]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (!src) return;
      const copy = Engine.cloneLanding(src);
      Store.saveLanding(copy);
      render();
    });
  });
}
...
function bindEdit(id) {
  document.getElementById("edit-form")?.addEventListener("submit", (e) => {
    ...
    ["headline", "subheadline", "cta", "problemTitle", "problemBody", "solutionTitle", "solutionBody", "storyTitle", "storyBody"].forEach(
      (k) => {
...
function render() {
  ...
  if (parts[0] === "new") {
    root.innerHTML = viewNew();
    bindNew();
    return;
  }
  if (parts[0] === "project" && parts[1]) {
    root.innerHTML = viewProject(parts[1]);
    bindProject(parts[1]);
    return;
  }
  if (parts[0] === "edit" && parts[1]) {
    root.innerHTML = viewEdit(parts[1]);
    bindEdit(parts[1]);
    return;
  }
